...
static FindBeaconSubHSMState_t CurrentState = DriveFWD; // <- change name to match ENUM
...
ES_Event RunFindBeaconSubHSM(ES_Event ThisEvent) {
    ...
    FindBeaconSubHSMState_t nextState; // <- change type to correct enum
    ...
    switch (CurrentState) {
        case DriveFWD: // in the first state, replace this with correct names
            ...
            switch (ThisEvent.EventType) {
                ...
                case FL_TAPE_ON:
                    nextState = BackUp;
                    makeTransition = TRUE;
                    ES_Timer_InitTimer(TOP_LEVEL_TIMER, 3500);
                    break;
                case FR_TAPE_ON:
                    nextState = BackUp;
                    makeTransition = TRUE;
        ...
        case TankTurnLeft: // in the first state, replace this with correct names
            ...
            break;
        case AlignLeft: // in the first state, replace this with correct names
            LeftTankTurn(700);
            switch (ThisEvent.EventType) {
                case ES_TIMEOUT:
                    nextState = TankTurnRight;
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunFindBeaconSubHSM(EXIT_EVENT); // <- rename to your own Run function
        CurrentState = nextState;
        RunFindBeaconSubHSM(ENTRY_EVENT); // <- rename to your own Run function
    }
